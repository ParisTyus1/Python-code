...
def over_nine_thousand(lst):
    sum = 0 # empty bucket
    for i in lst:  #Loop index of lst
        if sum == 9000:  #if sum of i = 9000 break, else sum < 9000 return sum
            break;
        elif sum < 9000:
            return sum;
...
#Write your function here
def max_num(nums):
    maximum = nums[0]  #create variable and set parameter nums to first element of array
    for number in nums:  #Loop
        if number > maximum: #if number is greater than max; max should reset to new number
            maximum = number
    return maximum
...
# Working on string review

# Copeland’s Corporate Company has realized that their policy
# of using the first five letters of an employee’s last name
# as a user name isn’t ideal when they have multiple employees
# with the same last name.

# Write a function called account_generator that takes two inputs,
# first_name and last_name and concatenates the first three
# letters of each and then returns the new account name.
